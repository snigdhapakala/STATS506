...
### <https://github.com/snigdhapakala/STATS506/tree/main>

##### I apologize, for this first homework I was still getting the hang of the flow and didn't frequently push to my repository as was recommended in class. I think I finally understand how the pushing process works as it's my first time working with version control, so I definitely will be using it as a way to keep track of changes and allow for proof of progress in a natural timely manner going forward.
...
wine_file <- read.csv("~/Desktop/STATS506/GitHub/506_HW1/wine/wine.data", sep=",", header = FALSE)
...
2.  Which class has the highest correlation? Which has the lowest?
    ...
    #'
    #' @param n represents each number for the wine classes
    #'
...
Answer for C4: 8.43% of wines had a higher content of proanthocyanins compare to ash.
...
survey_file <- read.csv("~/Desktop/STATS506/GitHub/506_HW1/AskAManager.csv", sep=",")
...
cat("With putting both constraints on our data, we now see the following number of rows:", nrow(survey_df_logical_income))
